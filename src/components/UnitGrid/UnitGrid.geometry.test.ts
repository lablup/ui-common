...
const place = (sizes: Record<string, number>, layout: UnitGridLayout, cols: number) =>
  placeCells(packGroups(makeGroups(sizes), 256), layout, cols, m);

const colOf = (cell: { px: number }) => Math.round((cell.px - m.pad) / m.stridePx);
...
describe("deriveMetrics", () => {
  ...
  it("derives lattice columns from width with a floor of 6", () => {
    ...
    expect(latticeColsForWidth(m.pad * 2 + 10 * m.stridePx - m.gapPx, m)).toBe(10);
  });
});
  ...
  it("keeps every group one 4-connected region across row wraps", () => {
    byGroup(placed).forEach((cells) => {
      for (let i = 1; i < cells.length; i++) {
        ...
        expect((dRow === 0 && dCol === 1) || (dRow === 1 && dCol === 0)).toBe(true);
      }
    });
  });
...
describe("letterCellIndices", () => {
  it("picks the visual top-left cell, not the flow-first, on serpentine", () => {
    ...
    const minPx = Math.min(...bCells.filter((c) => c.row === minRow).map((c) => c.px));
