/**
 * UnitGrid geometry: lattice packing (serpentine or word-wrap), per-group row
 * segments, boundary-loop chaining and rounded rectilinear plate paths. No
 * React and no DOM, so it is tested as plain data.
 */
...
export const latticeColsForWidth = (width: number, m: UnitGridMetrics): number =>
...
export const placeCells = <TUnit>(
  ...
  cells.forEach((cell) => {
    if (cell.first && slot > 0) {
      ...
      if (layout === "wordwrap" && n <= latticeCols && slot + 1 + n > latticeCols) {
        crow += 1;
        slot = 0;
      } else {
...
export const letterCellIndices = (
  ...
  placed.forEach((c, i) => {
    ...
    if (c.py < cur.py || (c.py === cur.py && c.px < cur.px)) indices.set(c.groupKey, i);
  });
  return indices;
};
