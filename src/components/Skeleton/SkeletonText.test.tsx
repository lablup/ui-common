...
/**
 * The width a line was asked for. Astryx `Skeleton` sets it as its own
 * `--x-width` custom property and sizes from that in `astryx.css`, which
 * jsdom does not load, so `toHaveStyle({ width })` cannot see it.
 */
function widthOf(line: Element | undefined): string {
  if (!(line instanceof HTMLElement)) throw new Error("no line rendered");
  return line.style.getPropertyValue("--x-width");
}
...
describe("SkeletonText", () => {
  describe("Rendering", () => {
    it("should render with default props", () => {
      ...
      const textContainer = container.querySelector(".uic-skeleton-text");
    ...
    it("should render 3 lines by default", () => {
      ...
      const lines = container.querySelectorAll(".uic-skeleton-shape");
  ...
  describe("Line Widths", () => {
    it("should have last line at 60% width", () => {
      ...
      expect(widthOf(lastLine)).toBe("60%");
    });
    ...
    it("should have first line at 100% width", () => {
      ...
      const lines = container.querySelectorAll(".uic-skeleton-shape");
      expect(widthOf(lines[0])).toBe("100%");
    });
    ...
    it("should alternate between 100% and 95% for middle lines", () => {
      ...
      // Line 4: 60% (last line)
      expect(widthOf(lines[0])).toBe("100%");
      expect(widthOf(lines[1])).toBe("95%");
      expect(widthOf(lines[2])).toBe("100%");
      expect(widthOf(lines[3])).toBe("95%");
      expect(widthOf(lines[4])).toBe("60%");
    });
    ...
    it("should handle single line (which is also last line)", () => {
      ...
      const lines = container.querySelectorAll(".uic-skeleton-shape");
      expect(widthOf(lines[0])).toBe("60%");
    });
  });
  ...
  describe("Spacing Variants", () => {
    it("should render with normal spacing by default", () => {
      ...
      const textContainer = container.querySelector(".uic-skeleton-text");
      expect(textContainer).toHaveClass("uic-skeleton-text--normal");
    });
    ...
    it("should render with compact spacing", () => {
      ...
      const textContainer = container.querySelector(".uic-skeleton-text");
      expect(textContainer).toHaveClass("uic-skeleton-text--compact");
    });
    ...
    it("should render with relaxed spacing", () => {
      ...
      const textContainer = container.querySelector(".uic-skeleton-text");
      expect(textContainer).toHaveClass("uic-skeleton-text--relaxed");
    });
  });

  describe("CSS Classes", () => {
    it("should include base uic-skeleton-text class", () => {
      const { container } = render(<SkeletonText />);

      const textContainer = container.querySelector(".uic-skeleton-text");
      expect(textContainer).toHaveClass("uic-skeleton-text");
    });
    ...
    it("should include custom className", () => {
      ...
      const textContainer = container.querySelector(".uic-skeleton-text");
      expect(textContainer).toHaveClass("uic-skeleton-text", "custom-class");
    });

    it("should render skeleton lines with text variant", () => {
      const { container } = render(<SkeletonText />);

      const lines = container.querySelectorAll(".uic-skeleton-shape--text");
  ...
  describe("Edge Cases", () => {
    it("should handle all props combined", () => {
      ...
      const lines = textContainer.querySelectorAll(".uic-skeleton-shape");
